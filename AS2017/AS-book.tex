...
%\usepackage{amssymb}
\usepackage{imakeidx}
%\usepackage[xindy]{imakeidx} % to try and solve author index special symbol problem, also check makeindex command
...
\def\width{15}
\def\hauteur{25}
%for HYPERREF and FANCYHDR package check way below
...
\newcommand{\Abstract}[1]{\par \vspace{16pt} \large{#1} \par }
...
\makeindex
% xindy index variant:
%\makeindex[title=\indexname,options=-L turkish -C utf8,columns=2]
...
xxxxxxxxxxxxx\=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\=xxxxxxxx\=xxxxxxxxxxxxxxxxxxxxxxxxxx \kill
%#1  \> \bf{#2} \> #3 \> {\em #4}\\[0.1mm]
#1  \> \bf{#2} \> \> \\
    \>  #3 \> {\em #4} \>\\[0.1mm]
\end{tabbing}
...
% comment/uncomment this to change the email/link-colours to default or black
...
%\noindent  \hrulefill \\
\vspace{3cm}
\hspace{1.5cm} \includegraphics{Sponsors-2017-crop.pdf}
